...
impl Transform for ScalarizeStructs {
    fn run(&self, func: &mut MirFunction) -> bool {
        ...
        for candidate in candidates {
            ...
            let mut sorted_fields: Vec<(&String, &(usize, crate::semantic::types::Type))> = field_map.iter().collect();
    ...
    for bb in &func.basic_blocks {
        for stmt in &bb.statements {
            ...
            match &stmt.kind {
                ...
                StatementKind::SetAttr(op, _, val) if operand_local(op).map_or(false, |l| aliases.contains(&l)) => {
                ...
                StatementKind::SetIndex(op, idx_op, val) if operand_local(op).map_or(false, |l| aliases.contains(&l)) => {
                    match idx_op {
                        Operand::Constant(Constant::Int(_)) | Operand::Constant(Constant::Str(_)) => {}
                        _ => return false,
                    }
                    if operand_local(val).map_or(false, |l| aliases.contains(&l)) {
                        return false;
                ...
                StatementKind::Assign(dst, Rvalue::Use(op))
                    if aliases.contains(dst) && operand_local(op).map_or(false, |l| aliases.contains(&l)) => {}
                ...
                _ => {
                    println!("Cannot scalarize alias {:?} because of stmt: {:?}", origin, stmt);
                    return false;
                }
            }
        }
...
fn collect_field_map(func: &MirFunction, aliases: &HashSet<Local>) -> HashMap<String, (usize, crate::semantic::types::Type)> {
                    ...
                    if operand_local(op).map_or(false, |l| aliases.contains(&l)) =>
                {
                    if !map.contains_key(field) {
                        let ty = match val {
                            ...
                            Operand::Constant(Constant::Float(_)) => crate::semantic::types::Type::Float,
...
fn rewrite(func: &mut MirFunction, aliases: &HashSet<Local>, origin: Local, field_map: &HashMap<String, (usize, crate::semantic::types::Type)>, base: usize) {
    for bb in &mut func.basic_blocks {
        ...
        for stmt in bb.statements.drain(..) {
            match stmt.kind {
                ...
                StatementKind::StorageLive(l) | StatementKind::StorageDead(l) if aliases.contains(&l) => {}
...
fn stmt_references(stmt: &Statement, local: Local) -> bool {
    match &stmt.kind {
        StatementKind::Assign(l, rval) => {
            *l == local || rval_references(rval, local)
        }
        ...
        StatementKind::Drop(l)
        | StatementKind::StorageLive(l)
        | StatementKind::StorageDead(l) => *l == local,
...
fn rval_references(rval: &Rvalue, local: Local) -> bool {
    match rval {
        ...
        | Rvalue::GetTag(op) | Rvalue::GetTypeId(op) | Rvalue::PtrLoad(op)
