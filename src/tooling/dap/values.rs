//! Variable rendering while the debuggee is parked. Leaf text comes from
//! the runtime's own `olive_format_typed`, resolved through
//! `runtime_symbol`; the object-graph reads that decision needs (list/set
//! length, dict entries, enum tag/payload) go through the D4 ABI helpers in
...
pub fn frame_variables(session: &DebugSession, frame_idx: usize) -> Vec<Variable> {
...
pub fn children(session: &DebugSession, reference: i64) -> Vec<Variable> {
    let Some(node) = session.var_store.get(reference) else {
        return Vec::new();
    };
    match concrete_ty(&node.ty) {
        Type::List(inner) | Type::Vector(inner, _) | Type::Set(inner) => {
            let n = seq_len(session, node.value);
            (0..n)
                .map(|i| {
                    let elem = seq_get(session, node.value, i);
                    describe_fresh(session, i.to_string(), inner, elem)
                })
                .collect()
        }
        Type::Tuple(items) => items
            ...
            .enumerate()
            .map(|(i, item_ty)| {
                let elem = seq_get(session, node.value, i as i64);
                describe_fresh(session, i.to_string(), item_ty, elem)
            })
            .collect(),
        Type::Dict(k, v) => {
            let n = dict_len(session, node.value);
            (0..n)
                .map(|i| {
                    let kraw = dict_key(session, node.value, i);
                    let vraw = dict_val(session, node.value, i);
                    let key_text = render_value(session, k, kraw);
                    describe_fresh(session, key_text, v, vraw)
                })
                .collect()
        }
            ...
            fields
                ...
                .map(|(i, fname)| {
                    let fty = session
                        ...
                        .cloned()
                        .unwrap_or(Type::Any);
                    let fval = if node.value == 0 {
                        0
                    } else {
                        unsafe { *(node.value as *const i64).add(1 + i) }
                    };
                    describe_fresh(session, fname.clone(), &fty, fval)
                })
                .collect()
        }
        Type::Enum(name, _) => {
            ...
            let tag = enum_tag(session, node.value);
            ...
            payload_tys
                ...
                .enumerate()
                .map(|(i, pty)| {
                    let pval = enum_payload(session, node.value, i as i64);
                    describe_fresh(session, i.to_string(), pty, pval)
                })
                .collect()
        }
        _ => Vec::new(),
...
fn describe_fresh(session: &DebugSession, name: String, ty: &Type, raw: i64) -> Variable {
...
fn render_value(session: &DebugSession, ty: &Type, raw: i64) -> String {
    let concrete = concrete_ty(ty);
    let desc = build_descriptor(session, concrete);
    format_typed(session, coerce_bits(concrete, raw), &desc)
}

fn build_descriptor(session: &DebugSession, ty: &Type) -> CString {
...
fn format_typed(session: &DebugSession, val: i64, desc: &CStr) -> String {
...
fn call1(session: &DebugSession, name: &str, a: i64) -> i64 {
...
fn call2(session: &DebugSession, name: &str, a: i64, b: i64) -> i64 {
    let Some(ptr) = session.runtime_symbol(name) else {
        return 0;
    };
    let f: extern "C" fn(i64, i64) -> i64 = unsafe { std::mem::transmute(ptr) };
...
fn seq_len(session: &DebugSession, val: i64) -> i64 {
    call1(session, "olive_debug_seq_len", val)
}

fn seq_get(session: &DebugSession, val: i64, idx: i64) -> i64 {
    call2(session, "olive_debug_seq_get", val, idx)
}

fn dict_len(session: &DebugSession, val: i64) -> i64 {
    call1(session, "olive_debug_dict_len", val)
}

fn dict_key(session: &DebugSession, val: i64, idx: i64) -> i64 {
    call2(session, "olive_debug_dict_key", val, idx)
}

fn dict_val(session: &DebugSession, val: i64, idx: i64) -> i64 {
    call2(session, "olive_debug_dict_val", val, idx)
}

fn enum_tag(session: &DebugSession, val: i64) -> i64 {
    call1(session, "olive_debug_enum_tag", val)
}

fn enum_payload(session: &DebugSession, val: i64, idx: i64) -> i64 {
