...
mod tests {
    ...
    /// The session must drop before its process-wide execution lock.
    fn stopped_session() -> (std::sync::MutexGuard<'static, ()>, DebugSession) {
    ...
    fn plain_ident_resolves_a_local() {
        let (_guard, session) = stopped_session();
