    ...
    pub(super) fn translate_binop(
        ...
        match op {
            ...
            In | NotIn => {
                ...
                // `Any`-keyed dicts and sets stay untyped: the MIR `in`
                // lowering boxes needles into the identical words the
                // stores hold, so the heuristic classifies every key the
                // same at store, lookup, and growth rehash.
