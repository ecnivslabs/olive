...
impl<M: Module> CraneliftCodegen<M> {
        ...
        for bb in &func.basic_blocks {
            for stmt in &bb.statements {
                match &stmt.kind {
                    ...
                    StatementKind::SetAttr(obj_op, attr, val_op) => {
    ...
    fn collect_type_descriptor(&mut self, func: &MirFunction, rval: &crate::mir::Rvalue) {
        ...
        let typed_list_arg = match name.as_str() {
            ...
            "__olive_obj_update_typed" | "__olive_obj_update_from_any" if args.len() == 2 => {
                Some(0usize)
            }
            "__olive_chan_send" | "__olive_mutex_unlock" if args.len() == 2 => Some(1usize),
            "__olive_obj_values_typed" | "__olive_obj_items_typed" if args.len() == 1 => {
                Some(0usize)
            }
            "__olive_obj_keys_typed" if args.len() == 1 => Some(0usize),
