use crate::mir::{Constant, MirFunction, Operand};
pub(crate) use crate::semantic::type_descriptor::{
    concrete_ty, needs_type_descriptor, type_descriptor,
};
use crate::semantic::types::Type as OliveType;
use cranelift::prelude::{FunctionBuilder, InstBuilder, Value, types};
use rustc_hash::FxHashMap as HashMap;

/// Typed zero (int/float/vector) for `ty`; SSA builder asserts the stored
/// value's type equals the variable's declared type.
pub(crate) fn typed_zero(builder: &mut FunctionBuilder, ty: cranelift::prelude::Type) -> Value {
    if ty == types::F64 {
        builder.ins().f64const(0.0)
    } else if ty == types::F32 {
        builder.ins().f32const(0.0)
    } else if ty.is_int() {
        builder.ins().iconst(ty, 0)
    } else if ty.is_vector() {
        let lane = ty.lane_type();
        let scalar = if lane == types::F64 {
            builder.ins().f64const(0.0)
        } else if lane == types::F32 {
            builder.ins().f32const(0.0)
        } else {
            builder.ins().iconst(lane, 0)
        };
        builder.ins().splat(ty, scalar)
    } else {
        builder.ins().iconst(types::I64, 0)
    }
}

/// Whether a dict key / set element needs a key descriptor on the runtime
/// op: aggregates need the structural hash+eq derived `==` uses, and a
/// concrete scalar key needs the descriptor so `classify_key` trusts the
/// static type over its string-pointer magnitude heuristic (a raw odd int
/// above the string-tag floor is bit-identical to a tagged string pointer).
/// Only `Any`-typed keys stay on the heuristic path.
pub(crate) fn needs_key_descriptor(ty: &OliveType) -> bool {
    if matches!(ty, OliveType::Union(members)
        if members.iter().filter(|m| !matches!(m, OliveType::Null)).count() == 1)
    {
        return true;
    }
    matches!(
        concrete_ty(ty),
        OliveType::Struct(..)
            | OliveType::Enum(..)
            | OliveType::Tuple(_)
            | OliveType::List(_)
            | OliveType::Set(_)
            | OliveType::Dict(_, _)
            | OliveType::Int
            | OliveType::I8
            | OliveType::I16
            | OliveType::I32
            | OliveType::U8
            | OliveType::U16
            | OliveType::U32
            | OliveType::U64
            | OliveType::Usize
            | OliveType::Float
            | OliveType::F32
            | OliveType::Str
            | OliveType::Bool
            | OliveType::Null
    )
}

/// Whether an `Any`-keyed container lookup with this index type takes the
/// index-derived typed path. Scalars only: aggregates keep the untyped
/// structural protocol (`Any`-form boxing plus the shape-aware hash and
/// equality helpers), which the typed descriptor path would disagree with.
pub(crate) fn scalar_needs_key_descriptor(ty: &OliveType) -> bool {
    if matches!(ty, OliveType::Union(members)
        if members.iter().filter(|m| !matches!(m, OliveType::Null)).count() == 1)
    {
        return true;
    }
    matches!(
        concrete_ty(ty),
        OliveType::Int
            | OliveType::I8
            | OliveType::I16
            | OliveType::I32
            | OliveType::U8
            | OliveType::U16
            | OliveType::U32
            | OliveType::U64
            | OliveType::Usize
            | OliveType::Float
            | OliveType::F32
            | OliveType::Str
            | OliveType::Bool
            | OliveType::Null
    )
}

/// An operand's static type for descriptor purposes: a `Copy`/`Move` reads
/// its local's declared type; a `Constant` infers the literal's own type.
/// Shared by codegen's call translation and the pre-pass that interns every
/// descriptor string it will need -- the two must agree on every operand
/// shape or a descriptor goes missing (a constant-propagated `None` arg to
/// `__olive_copy_typed` is exactly this: the collector used to only look at
/// `Copy`/`Move`, silently skipping a folded `Constant` arg).
pub(crate) fn operand_static_type(op: &Operand, func_mir: &MirFunction) -> OliveType {
    match op {
        Operand::Constant(Constant::Str(_)) => OliveType::Str,
        Operand::Constant(Constant::Float(_)) => OliveType::Float,
        Operand::Constant(Constant::Bool(_)) => OliveType::Bool,
        Operand::Constant(Constant::None) => OliveType::Null,
        Operand::Copy(l) | Operand::Move(l) => func_mir.locals[l.0].ty.clone(),
        _ => OliveType::Int,
    }
}

/// Whether a runtime callee returns a container slot word that may hold
/// float bits (`d[k]`, `d.get(k)`, `xs[i]`, `.pop()`, `.setdefault()`).
/// Such words reach `F32` destinations as zero-extended f32 bits and must
/// narrow-plus-bitcast, never go through the numeric conversion or the
/// fused py-call `f64` path (which both read the bit pattern as a value).
/// Shared by the `Assign` fixup and the post-call fixup so the two agree.
pub(crate) fn is_container_read_call(name: &str) -> bool {
    matches!(
        name,
        "__olive_obj_get"
            | "__olive_obj_get_typed"
            | "__olive_obj_get_default"
            | "__olive_obj_get_default_typed"
            | "__olive_obj_get_default_boxed"
            | "__olive_obj_get_default_boxed_typed"
            | "__olive_obj_pop_checked"
            | "__olive_obj_pop_checked_typed"
            | "__olive_obj_pop_default"
            | "__olive_obj_pop_default_typed"
            | "__olive_obj_setdefault"
            | "__olive_obj_setdefault_typed"
            | "__olive_list_pop"
            | "__olive_list_remove"
            | "__olive_next"
    )
}

type StructFields = HashMap<String, Vec<String>>;

/// The type whose descriptor drives a typed drop, if the dropped type has a
/// useful one. A `T | None` union reduces to `T`; a struct without a known
/// field layout encodes as an opaque byte the typed free cannot act on, so it
/// stays on the untyped kind-dispatch path.
pub(crate) fn drop_descriptor_type<'a>(
    ty: &'a OliveType,
    struct_fields: &StructFields,
) -> Option<&'a OliveType> {
    let t = match ty {
        OliveType::Union(members) => {
            let non_null: Vec<&OliveType> = members
                .iter()
                .filter(|m| !matches!(m, OliveType::Null))
                .collect();
            match non_null.as_slice() {
                [single] => *single,
                _ => return None,
            }
        }
        other => other,
    };
    match t {
        OliveType::Struct(name, _, _) => struct_fields.contains_key(name).then_some(t),
        // A buffer has no element layout to walk; the direct free suffices.
        OliveType::Bytes => None,
        _ if needs_type_descriptor(t) => Some(t),
        _ => None,
    }
}

pub(super) static KNOWN_RUNTIME_IMPORTS: [&str; 731] = [
    "__olive_alloc",
    "__olive_any_add",
    "__olive_any_check_method",
    "__olive_any_clear",
    "__olive_any_count",
    "__olive_any_div",
    "__olive_any_eq",
    "__olive_any_eq_strict",
    "__olive_any_ge",
    "__olive_any_getattr",
    "__olive_any_gt",
    "__olive_any_is_null",
    "__olive_any_is_struct_box",
    "__olive_any_le",
    "__olive_any_lt",
    "__olive_any_mod",
    "__olive_any_mul",
    "__olive_any_ne",
    "__olive_any_ne_strict",
    "__olive_any_pop",
    "__olive_any_remove",
    "__olive_any_setattr",
    "__olive_any_sub",
    "__olive_any_to_str",
    "__olive_any_truthy",
    "__olive_assert_fail",
    "__olive_async_file_read",
    "__olive_async_file_write",
    "__olive_atexit",
    "__olive_atomic_add",
    "__olive_atomic_cas",
    "__olive_atomic_free",
    "__olive_atomic_get",
    "__olive_atomic_new",
    "__olive_atomic_set",
    "__olive_await",
    "__olive_base64_decode",
    "__olive_base64_encode",
    "__olive_base64_encode_bytes",
    "__olive_bool",
    "__olive_bool_from_float",
    "__olive_bool_to_str",
    "__olive_box_bool",
    "__olive_box_float",
    "__olive_box_int",
    "__olive_box_null",
    "__olive_box_u64",
    "__olive_buf_concat",
    "__olive_buf_free",
    "__olive_buf_from_base64",
    "__olive_buf_from_str",
    "__olive_buf_get",
    "__olive_buf_getslice",
    "__olive_buf_len",
    "__olive_buf_new",
    "__olive_buf_new_zeroed",
    "__olive_buf_push",
    "__olive_buf_push_u16_le",
    "__olive_buf_push_u32_le",
    "__olive_buf_read_file",
    "__olive_buf_read_u16_be",
    "__olive_buf_read_u16_le",
    "__olive_buf_read_u32_be",
    "__olive_buf_read_u32_le",
    "__olive_buf_read_u64_be",
    "__olive_buf_read_u64_le",
    "__olive_buf_set",
    "__olive_buf_slice",
    "__olive_buf_to_base64",
    "__olive_buf_to_hex",
    "__olive_buf_to_str",
    "__olive_buf_write_file",
    "__olive_buf_write_u16_be",
    "__olive_buf_write_u16_le",
    "__olive_buf_write_u32_be",
    "__olive_buf_write_u32_le",
    "__olive_buf_write_u64_be",
    "__olive_buf_write_u64_le",
    "__olive_bufread_close",
    "__olive_bufread_line",
    "__olive_bufread_open",
    "__olive_bufwrite_close",
    "__olive_bufwrite_flush",
    "__olive_bufwrite_open",
    "__olive_bufwrite_write",
    "__olive_cache_get",
    "__olive_cache_get_tuple",
    "__olive_cache_has",
    "__olive_cache_has_tuple",
    "__olive_cache_set",
    "__olive_cache_set_tuple",
    "__olive_calloc",
    "__olive_cancel_future",
    "__olive_chan_close",
    "__olive_chan_free",
    "__olive_chan_len",
    "__olive_chan_new",
    "__olive_chan_recv",
    "__olive_chan_send",
    "__olive_chan_try_recv",
    "__olive_check_list_min_len",
    "__olive_check_nonzero_step",
    "__olive_copy",
    "__olive_copy_dir",
    "__olive_copy_float",
    "__olive_crypto_aes_decrypt",
    "__olive_crypto_aes_encrypt",
    "__olive_crypto_argon2_hash",
    "__olive_crypto_argon2_verify",
    "__olive_crypto_md5",
    "__olive_crypto_rsa_decrypt",
    "__olive_crypto_rsa_encrypt",
    "__olive_crypto_rsa_keygen",
    "__olive_crypto_sha256",
    "__olive_datetime_add_days",
    "__olive_datetime_add_hours",
    "__olive_datetime_add_minutes",
    "__olive_datetime_add_months",
    "__olive_datetime_add_seconds",
    "__olive_datetime_add_years",
    "__olive_datetime_days_in_month",
    "__olive_datetime_diff_days",
    "__olive_datetime_diff_seconds",
    "__olive_datetime_end_of_day",
    "__olive_datetime_format",
    "__olive_datetime_from_local",
    "__olive_datetime_from_parts",
    "__olive_datetime_is_leap_year",
    "__olive_datetime_local_offset",
    "__olive_datetime_month_name",
    "__olive_datetime_now",
    "__olive_datetime_parse",
    "__olive_datetime_parts",
    "__olive_datetime_start_of_day",
    "__olive_datetime_start_of_month",
    "__olive_datetime_to_local",
    "__olive_datetime_utcnow",
    "__olive_datetime_weekday",
    "__olive_datetime_weekday_name",
    "__olive_debug_enter",
    "__olive_debug_exit",
    "__olive_debug_load",
    "__olive_debug_should_check_stmt",
    "__olive_debug_stmt",
    "__olive_debug_store",
    "__olive_dict_drop_each_struct",
    "__olive_dict_drop_each_union",
    "__olive_dir_create",
    "__olive_dir_list",
    "__olive_dlpack_bits",
    "__olive_dlpack_byte_offset",
    "__olive_dlpack_data_ptr",
    "__olive_dlpack_device_id",
    "__olive_dlpack_device_type",
    "__olive_dlpack_dtype_code",
    "__olive_dlpack_export",
    "__olive_dlpack_import",
    "__olive_dlpack_lanes",
    "__olive_dlpack_ndim",
    "__olive_dlpack_release",
    "__olive_dlpack_shape_at",
    "__olive_dlpack_strides_at",
    "__olive_enum_get",
    "__olive_enum_new",
    "__olive_enum_set",
    "__olive_enum_set_typed",
    "__olive_enum_tag",
    "__olive_enum_type_id",
    "__olive_env_get",
    "__olive_env_set",
    "__olive_export_buffer_view",
    "__olive_ffi_clear_errno",
    "__olive_ffi_errmsg",
    "__olive_ffi_errno",
    "__olive_file_append",
    "__olive_file_append_str",
    "__olive_file_close",
    "__olive_file_copy",
    "__olive_file_delete",
    "__olive_file_exists",
    "__olive_file_open",
    "__olive_file_read",
    "__olive_file_read_all",
    "__olive_file_read_lines",
    "__olive_file_read_n",
    "__olive_file_rename",
    "__olive_file_seek",
    "__olive_file_stat",
    "__olive_file_tell",
    "__olive_file_write",
    "__olive_file_write_str",
    "__olive_float",
    "__olive_float_to_int",
    "__olive_float_to_str",
    "__olive_format_any",
    "__olive_format_bool",
    "__olive_format_float",
    "__olive_format_int",
    "__olive_format_str",
    "__olive_format_u64",
    "__olive_free",
    "__olive_free_any",
    "__olive_free_future",
    "__olive_free_iter",
    "__olive_free_list",
    "__olive_free_obj",
    "__olive_free_str",
    "__olive_free_struct",
    "__olive_free_typed",
    "__olive_gather",
    "__olive_get_index_any",
    "__olive_get_index_any_typed",
    "__olive_getslice_any",
    "__olive_gzip_compress",
    "__olive_gzip_decompress",
    "__olive_has_next",
    "__olive_hex_decode",
    "__olive_hex_encode",
    "__olive_hook_union_word",
    "__olive_http_delete",
    "__olive_http_get",
    "__olive_http_get_status",
    "__olive_http_get_with_headers",
    "__olive_http_last_error",
    "__olive_http_poll",
    "__olive_http_post",
    "__olive_http_post_json",
    "__olive_http_post_json_async",
    "__olive_http_post_json_async_headers",
    "__olive_http_put",
    "__olive_http_str_is_null",
    "__olive_http_stream_close",
    "__olive_http_stream_poll",
    "__olive_http_stream_start",
    "__olive_http_stream_take_chunk",
    "__olive_http_stream_take_error",
    "__olive_http_take_error",
    "__olive_http_take_result",
    "__olive_in_list",
    "__olive_in_list_typed",
    "__olive_in_obj",
    "__olive_in_obj_typed",
    "__olive_int",
    "__olive_int_abs",
    "__olive_int_to_float",
    "__olive_is_bytes",
    "__olive_is_list",
    "__olive_is_null",
    "__olive_is_obj",
    "__olive_is_str",
    "__olive_iter",
    "__olive_iter_typed",
    "__olive_json_parse",
    "__olive_json_stringify",
    "__olive_json_stringify_pretty",
    "__olive_len_any",
    "__olive_list_all_any",
    "__olive_list_all_bool",
    "__olive_list_any_any",
    "__olive_list_any_bool",
    "__olive_list_append",
    "__olive_list_apply_order",
    "__olive_list_clear",
    "__olive_list_clear_typed",
    "__olive_list_concat",
    "__olive_list_concat_typed",
    "__olive_list_count_typed",
    "__olive_list_drop_each_struct",
    "__olive_list_drop_each_union",
    "__olive_list_extend",
    "__olive_list_extend_typed",
    "__olive_list_get",
    "__olive_list_getslice",
    "__olive_list_getslice_typed",
    "__olive_list_index_typed",
    "__olive_list_insert",
    "__olive_list_len",
    "__olive_list_mark_any",
    "__olive_list_new",
    "__olive_list_pop",
    "__olive_list_remove",
    "__olive_list_repeat",
    "__olive_list_repeat_typed",
    "__olive_list_reverse",
    "__olive_list_set",
    "__olive_list_set_typed",
    "__olive_list_sort_any",
    "__olive_list_sort_by_keys",
    "__olive_list_sort_f32",
    "__olive_list_sort_float",
    "__olive_list_sort_int",
    "__olive_list_sort_str",
    "__olive_list_unerase",
    "__olive_log_clear_fields",
    "__olive_log_debug",
    "__olive_log_error",
    "__olive_log_info",
    "__olive_log_level_from_str",
    "__olive_log_set_format",
    "__olive_log_set_level",
    "__olive_log_warn",
    "__olive_log_with_field",
    "__olive_make_future",
    "__olive_math_acos",
    "__olive_math_asin",
    "__olive_math_atan",
    "__olive_math_atan2",
    "__olive_math_cos",
    "__olive_math_exp",
    "__olive_math_log",
    "__olive_math_log10",
    "__olive_math_sin",
    "__olive_math_tan",
    "__olive_math_wrapping_add",
    "__olive_math_wrapping_mul",
    "__olive_memo_get",
    "__olive_mutex_free",
    "__olive_mutex_lock",
    "__olive_mutex_new",
    "__olive_mutex_unlock",
    "__olive_net_dns_lookup",
    "__olive_net_dns_lookup_all",
    "__olive_net_tcp_accept",
    "__olive_net_tcp_close",
    "__olive_net_tcp_connect",
    "__olive_net_tcp_listen",
    "__olive_net_tcp_listener_addr",
    "__olive_net_tcp_listener_close",
    "__olive_net_tcp_peer_addr",
    "__olive_net_tcp_recv",
    "__olive_net_tcp_send",
    "__olive_net_tcp_set_timeout",
    "__olive_net_udp_close",
    "__olive_net_udp_open",
    "__olive_net_udp_recv",
    "__olive_net_udp_send",
    "__olive_net_udp_set_timeout",
    "__olive_next",
    "__olive_none_to_str",
    "__olive_obj_clear",
    "__olive_obj_clear_typed",
    "__olive_obj_get",
    "__olive_obj_get_boxed",
    "__olive_obj_get_checked",
    "__olive_obj_get_default",
    "__olive_obj_get_default_boxed",
    "__olive_obj_items",
    "__olive_obj_items_typed",
    "__olive_obj_keys",
    "__olive_obj_keys_typed",
    "__olive_obj_len",
    "__olive_obj_new",
    "__olive_obj_pop_checked",
    "__olive_obj_pop_default",
    "__olive_obj_remove",
    "__olive_obj_set",
    "__olive_obj_set_replacing_typed",
    "__olive_obj_setdefault",
    "__olive_obj_unerase",
    "__olive_obj_update",
    "__olive_obj_values",
    "__olive_obj_values_typed",
    "__olive_os_args",
    "__olive_os_exit",
    "__olive_panic",
    "__olive_path_basename",
    "__olive_path_canonicalize",
    "__olive_path_dirname",
    "__olive_path_ext",
    "__olive_path_hard_link",
    "__olive_path_is_absolute",
    "__olive_path_is_symlink",
    "__olive_path_join",
    "__olive_path_join_all",
    "__olive_path_read_link",
    "__olive_path_relative",
    "__olive_path_stem",
    "__olive_path_symlink",
    "__olive_pool_run",
    "__olive_pool_run_sync",
    "__olive_pool_size",
    "__olive_pow",
    "__olive_print_any",
    "__olive_print_enum",
    "__olive_print_float",
    "__olive_print_int",
    "__olive_print_list",
    "__olive_print_list_float",
    "__olive_print_obj",
    "__olive_print_py",
    "__olive_print_str",
    "__olive_print_u64",
    "__olive_process_close",
    "__olive_process_close_stdin",
    "__olive_process_exit_code",
    "__olive_process_kill",
    "__olive_process_pid",
    "__olive_process_poll",
    "__olive_process_read_stderr",
    "__olive_process_read_stdout",
    "__olive_process_shell_argv",
    "__olive_process_signal_code",
    "__olive_process_spawn",
    "__olive_process_stderr_truncated",
    "__olive_process_stdout_truncated",
    "__olive_process_terminate",
    "__olive_process_wait",
    "__olive_process_wait_timeout",
    "__olive_process_write_stdin",
    "__olive_py_add",
    "__olive_py_bitor",
    "__olive_py_call",
    "__olive_py_call0",
    "__olive_py_call0_safe",
    "__olive_py_call1",
    "__olive_py_call1_safe",
    "__olive_py_call2",
    "__olive_py_call2_safe",
    "__olive_py_call3",
    "__olive_py_call3_safe",
    "__olive_py_call4",
    "__olive_py_call4_safe",
    "__olive_py_call_kw",
    "__olive_py_call_kw_safe",
    "__olive_py_call_kw_v",
    "__olive_py_call_kw_v_p0_k1",
    "__olive_py_call_kw_v_p0_k1_safe",
    "__olive_py_call_kw_v_p0_k2",
    "__olive_py_call_kw_v_p0_k2_safe",
    "__olive_py_call_kw_v_p0_k3",
    "__olive_py_call_kw_v_p0_k3_safe",
    "__olive_py_call_kw_v_p0_k4",
    "__olive_py_call_kw_v_p0_k4_safe",
    "__olive_py_call_kw_v_p1_k1",
    "__olive_py_call_kw_v_p1_k1_safe",
    "__olive_py_call_kw_v_p1_k2",
    "__olive_py_call_kw_v_p1_k2_safe",
    "__olive_py_call_kw_v_p1_k3",
    "__olive_py_call_kw_v_p1_k3_safe",
    "__olive_py_call_kw_v_p2_k1",
    "__olive_py_call_kw_v_p2_k1_safe",
    "__olive_py_call_kw_v_p2_k2",
    "__olive_py_call_kw_v_p2_k2_safe",
    "__olive_py_call_kw_v_p3_k1",
    "__olive_py_call_kw_v_p3_k1_safe",
    "__olive_py_call_kw_v_safe",
    "__olive_py_call_method0",
    "__olive_py_call_method0_safe",
    "__olive_py_call_method1",
    "__olive_py_call_method1_safe",
    "__olive_py_call_method2",
    "__olive_py_call_method2_safe",
    "__olive_py_call_method3",
    "__olive_py_call_method3_safe",
    "__olive_py_call_method4",
    "__olive_py_call_method4_safe",
    "__olive_py_call_method_kw_v",
    "__olive_py_call_method_kw_v_p0_k1",
    "__olive_py_call_method_kw_v_p0_k1_safe",
    "__olive_py_call_method_kw_v_p0_k2",
    "__olive_py_call_method_kw_v_p0_k2_safe",
    "__olive_py_call_method_kw_v_p0_k3",
    "__olive_py_call_method_kw_v_p0_k3_safe",
    "__olive_py_call_method_kw_v_p0_k4",
    "__olive_py_call_method_kw_v_p0_k4_safe",
    "__olive_py_call_method_kw_v_p1_k1",
    "__olive_py_call_method_kw_v_p1_k1_safe",
    "__olive_py_call_method_kw_v_p1_k2",
    "__olive_py_call_method_kw_v_p1_k2_safe",
    "__olive_py_call_method_kw_v_p1_k3",
    "__olive_py_call_method_kw_v_p1_k3_safe",
    "__olive_py_call_method_kw_v_p2_k1",
    "__olive_py_call_method_kw_v_p2_k1_safe",
    "__olive_py_call_method_kw_v_p2_k2",
    "__olive_py_call_method_kw_v_p2_k2_safe",
    "__olive_py_call_method_kw_v_p3_k1",
    "__olive_py_call_method_kw_v_p3_k1_safe",
    "__olive_py_call_method_kw_v_safe",
    "__olive_py_call_safe",
    "__olive_py_call_t",
    "__olive_py_call_t_safe",
    "__olive_py_copy_ref",
    "__olive_py_create_module",
    "__olive_py_decref",
    "__olive_py_div",
    "__olive_py_eq",
    "__olive_py_finalize",
    "__olive_py_from_float",
    "__olive_py_from_int",
    "__olive_py_from_str",
    "__olive_py_from_typed",
    "__olive_py_from_u64",
    "__olive_py_getattr",
    "__olive_py_getattr_ret",
    "__olive_py_getattr_safe",
    "__olive_py_getitem",
    "__olive_py_getitem_int",
    "__olive_py_getitem_safe",
    "__olive_py_getslice",
    "__olive_py_gil_begin",
    "__olive_py_gil_checkpoint",
    "__olive_py_gil_end",
    "__olive_py_import",
    "__olive_py_import_safe",
    "__olive_py_initialize",
    "__olive_py_is_handle",
    "__olive_py_is_none",
    "__olive_py_len",
    "__olive_py_make_callable",
    "__olive_py_make_export",
    "__olive_py_mod",
    "__olive_py_module_add_object",
    "__olive_py_mul",
    "__olive_py_noconvert",
    "__olive_py_none",
    "__olive_py_pow",
    "__olive_py_set_loc",
    "__olive_py_setattr",
    "__olive_py_setattr_safe",
    "__olive_py_setitem",
    "__olive_py_setitem_int",
    "__olive_py_setitem_safe",
    "__olive_py_sub",
    "__olive_py_to_any",
    "__olive_py_to_any_dict",
    "__olive_py_to_any_list",
    "__olive_py_to_any_set",
    "__olive_py_to_bytes",
    "__olive_py_to_dict",
    "__olive_py_to_dict_typed",
    "__olive_py_to_float",
    "__olive_py_to_int",
    "__olive_py_to_list",
    "__olive_py_to_set",
    "__olive_py_to_set_typed",
    "__olive_py_to_str",
    "__olive_py_to_typed_arg",
    "__olive_random_get",
    "__olive_random_int",
    "__olive_random_seed",
    "__olive_range_list",
    "__olive_regex_captures",
    "__olive_regex_compile",
    "__olive_regex_find",
    "__olive_regex_find_all",
    "__olive_regex_is_valid",
    "__olive_regex_match",
    "__olive_regex_pattern_capture_named",
    "__olive_regex_pattern_captures",
    "__olive_regex_pattern_captures_all",
    "__olive_regex_pattern_close",
    "__olive_regex_pattern_find",
    "__olive_regex_pattern_find_all",
    "__olive_regex_pattern_matches",
    "__olive_regex_pattern_replace",
    "__olive_regex_pattern_replace_all",
    "__olive_regex_pattern_split",
    "__olive_regex_replace",
    "__olive_regex_replace_all",
    "__olive_regex_split",
    "__olive_register_drop",
    "__olive_relocate_any",
    "__olive_result_err",
    "__olive_result_err_msg",
    "__olive_result_is_err",
    "__olive_result_is_ok",
    "__olive_result_ok",
    "__olive_result_unwrap",
    "__olive_result_unwrap_err",
    "__olive_result_unwrap_or",
    "__olive_run_exit_hooks",
    "__olive_select",
    "__olive_set_add",
    "__olive_set_clear",
    "__olive_set_clear_typed",
    "__olive_set_contains",
    "__olive_set_diff",
    "__olive_set_diff_typed",
    "__olive_set_drop_each_struct",
    "__olive_set_drop_each_union",
    "__olive_set_fault_loc",
    "__olive_set_index_any",
    "__olive_set_index_any_typed",
    "__olive_set_intersection",
    "__olive_set_intersection_typed",
    "__olive_set_mode",
    "__olive_set_mtime",
    "__olive_set_new",
    "__olive_set_remove",
    "__olive_set_remove_checked",
    "__olive_set_sym_diff",
    "__olive_set_sym_diff_typed",
    "__olive_set_unerase",
    "__olive_set_union",
    "__olive_set_union_typed",
    "__olive_shadow_pop",
    "__olive_shadow_push",
    "__olive_shared_struct_alloc",
    "__olive_signal_install_sigint",
    "__olive_sm_alloc",
    "__olive_sm_free",
    "__olive_sm_poll",
    "__olive_spawn_task",
    "__olive_stdin_read",
    "__olive_stdin_read_line",
    "__olive_str",
    "__olive_str_capitalize",
    "__olive_str_center",
    "__olive_str_char",
    "__olive_str_char_count",
    "__olive_str_cmp",
    "__olive_str_concat",
    "__olive_str_concat_move",
    "__olive_str_contains",
    "__olive_str_count",
    "__olive_str_ends_with",
    "__olive_str_eq",
    "__olive_str_find",
    "__olive_str_fmt",
    "__olive_str_gen_of",
    "__olive_str_gen_stale",
    "__olive_str_get",
    "__olive_str_get_checked",
    "__olive_str_getslice",
    "__olive_str_grapheme_count",
    "__olive_str_graphemes",
    "__olive_str_is_ascii",
    "__olive_str_isalpha",
    "__olive_str_isdigit",
    "__olive_str_islower",
    "__olive_str_isspace",
    "__olive_str_isupper",
    "__olive_str_join",
    "__olive_str_len",
    "__olive_str_ljust",
    "__olive_str_lower",
    "__olive_str_partition",
    "__olive_str_removeprefix",
    "__olive_str_removesuffix",
    "__olive_str_repeat",
    "__olive_str_replace",
    "__olive_str_rfind",
    "__olive_str_rjust",
    "__olive_str_slice",
    "__olive_str_split",
    "__olive_str_splitlines",
    "__olive_str_starts_with",
    "__olive_str_title",
    "__olive_str_to_float",
    "__olive_str_to_float_opt",
    "__olive_str_to_int",
    "__olive_str_to_int_opt",
    "__olive_str_trim",
    "__olive_str_trim_chars",
    "__olive_str_trim_end",
    "__olive_str_trim_end_chars",
    "__olive_str_trim_start",
    "__olive_str_trim_start_chars",
    "__olive_str_u64",
    "__olive_str_upper",
    "__olive_str_zfill",
    "__olive_strbuf_build",
    "__olive_strbuf_char_len",
    "__olive_strbuf_clear",
    "__olive_strbuf_free",
    "__olive_strbuf_len",
    "__olive_strbuf_new",
    "__olive_strbuf_push",
    "__olive_struct_alloc",
    "__olive_struct_box",
    "__olive_struct_gate",
    "__olive_struct_gen_of",
    "__olive_struct_gen_stale",
    "__olive_struct_unbox",
    "__olive_struct_unbox_take",
    "__olive_sys_arch",
    "__olive_sys_chdir",
    "__olive_sys_cpu_count",
    "__olive_sys_cwd",
    "__olive_sys_home_dir",
    "__olive_sys_hostname",
    "__olive_sys_memory_free",
    "__olive_sys_memory_total",
    "__olive_sys_pid",
    "__olive_sys_platform",
    "__olive_sys_uptime",
    "__olive_sys_username",
    "__olive_temp_dir",
    "__olive_temp_file",
    "__olive_time_format",
    "__olive_time_monotonic",
    "__olive_time_now",
    "__olive_time_sleep",
    "__olive_to_py_typed",
    "__olive_to_pyobject",
    "__olive_toml_parse",
    "__olive_toml_stringify",
    "__olive_tuple_set_typed",
    "__olive_tuple_take",
    "__olive_tuple_unerase",
    "__olive_typeof_str",
    "__olive_unbox_float",
    "__olive_unbox_float_checked",
    "__olive_unbox_int",
    "__olive_unbox_int_checked",
    "__olive_url_decode",
    "__olive_url_encode",
    "__olive_uuid_is_valid",
    "__olive_uuid_nil",
    "__olive_uuid_to_hex",
    "__olive_uuid_v4",
    "__olive_walk",
    "__olive_websocket_close",
    "__olive_websocket_connect",
    "__olive_websocket_recv",
    "__olive_websocket_recv_binary",
    "__olive_websocket_send",
    "__olive_websocket_send_binary",
    "__olive_write_any",
    "__olive_write_bool",
    "__olive_write_char",
    "__olive_write_float",
    "__olive_write_int",
    "__olive_write_nl",
    "__olive_write_str",
    "__olive_write_typed",
    "__olive_write_u64",
    "__olive_yaml_parse",
    "__olive_yaml_stringify",
    "__olive_zstd_compress",
    "__olive_zstd_decompress",
];

pub(crate) fn resolve_builtin_import(
    func_mir: &MirFunction,
    name: &str,
    args: &[Operand],
) -> Option<&'static str> {
    if name.starts_with("__olive_") {
        return KNOWN_RUNTIME_IMPORTS
            .binary_search(&name)
            .ok()
            .map(|i| KNOWN_RUNTIME_IMPORTS[i]);
    }
    if name == "ffi_errno" {
        return Some("__olive_ffi_errno");
    }
    if name == "round" && !args.is_empty() {
        let resolved = if args.len() == 2 {
            "__olive_math_round_with_digits"
        } else {
            "__olive_math_round_to_int"
        };
        return Some(resolved);
    }
    if name == "input" {
        return if args.is_empty() {
            Some("__olive_stdin_read_line")
        } else {
            Some("__olive_input")
        };
    }
    match name {
        "print" | "str" | "int" | "float" | "bool" | "iter" | "next" | "has_next" | "len"
        | "slice" | "list" | "dict" | "sum" | "min" | "max" | "abs"
            if !args.is_empty() =>
        {
            let arg_type = match &args[0] {
                Operand::Constant(Constant::Str(_)) => OliveType::Str,
                Operand::Constant(Constant::Float(_)) => OliveType::Float,
                Operand::Constant(Constant::Bool(_)) => OliveType::Bool,
                Operand::Constant(Constant::None) => OliveType::Null,
                Operand::Copy(l) | Operand::Move(l) => func_mir.locals[l.0].ty.clone(),
                _ => OliveType::Int,
            };
            map_builtin_to_runtime(name, &arg_type)
        }

        "list_new" => Some("__olive_list_new"),
        _ => None,
    }
}

pub(crate) fn map_builtin_to_runtime(name: &str, arg_ty: &OliveType) -> Option<&'static str> {
    // Tag-encoded unions must keep their union identity here; the
    // concrete_ty collapse would route them to raw-word helpers.
    let current_ty = if arg_ty.is_tag_encoded_union() {
        arg_ty
    } else {
        concrete_ty(arg_ty)
    };

    match name {
        "len" => match current_ty {
            OliveType::Str => Some("__olive_str_len"),
            OliveType::Dict(_, _) | OliveType::Struct(_, _, _) => Some("__olive_obj_len"),
            // `Any` carries any word (a previous union narrow, an inlined
            // relay, a dynamic slot): only the total kind dispatch reads
            // every shape safely. `obj_len` assumes a dict header.
            OliveType::Any => Some("__olive_len_any"),
            // A wider union holds whichever member is live; measure by
            // runtime kind instead of guessing a layout statically.
            OliveType::Union(members)
                if members
                    .iter()
                    .filter(|m| !matches!(m, OliveType::Null))
                    .count()
                    > 1 =>
            {
                Some("__olive_len_any")
            }
            _ => Some("__olive_list_len"),
        },
        "sum" => match current_ty {
            OliveType::List(inner) if matches!(inner.as_ref(), OliveType::Float) => {
                Some("__olive_list_sum_float")
            }
            OliveType::List(inner) if matches!(inner.as_ref(), OliveType::F32) => {
                Some("__olive_list_sum_f32")
            }
            _ => Some("__olive_list_sum_int"),
        },
        "min" => match current_ty {
            OliveType::List(inner) if matches!(inner.as_ref(), OliveType::Float) => {
                Some("__olive_list_min_float")
            }
            OliveType::List(inner) if matches!(inner.as_ref(), OliveType::F32) => {
                Some("__olive_list_min_f32")
            }
            _ => Some("__olive_list_min_int"),
        },
        "max" => match current_ty {
            OliveType::List(inner) if matches!(inner.as_ref(), OliveType::Float) => {
                Some("__olive_list_max_float")
            }
            OliveType::List(inner) if matches!(inner.as_ref(), OliveType::F32) => {
                Some("__olive_list_max_f32")
            }
            _ => Some("__olive_list_max_int"),
        },
        "print" => match current_ty {
            OliveType::Str => Some("__olive_print_str"),
            OliveType::Float | OliveType::F32 => Some("__olive_print_float"),
            OliveType::List(inner)
                if matches!(inner.as_ref(), OliveType::Float | OliveType::F32) =>
            {
                Some("__olive_print_list_float")
            }
            OliveType::List(_) | OliveType::Tuple(_) | OliveType::Set(_) => {
                Some("__olive_print_list")
            }
            OliveType::Enum(_, _) => Some("__olive_print_enum"),
            OliveType::Bool => Some("__olive_print_bool"),
            OliveType::U64 => Some("__olive_print_u64"),
            OliveType::PyObject | OliveType::PyNamed(_, _) => Some("__olive_print_py"),
            OliveType::Union(_) | OliveType::Any => Some("__olive_print_any"),
            OliveType::Dict(_, _) | OliveType::Struct(_, _, _) => Some("__olive_print_obj"),
            OliveType::Null => Some("__olive_print_str"),
            _ => Some("__olive_print_int"),
        },
        "str" => match current_ty {
            OliveType::Str => Some("__olive_copy"),
            OliveType::Float | OliveType::F32 => Some("__olive_float_to_str"),
            OliveType::PyObject => Some("__olive_py_to_str"),
            OliveType::Any => Some("__olive_any_to_str"),
            OliveType::Union(_) if current_ty.is_tag_encoded_union() => Some("__olive_any_to_str"),
            OliveType::Null => Some("__olive_none_to_str"),
            OliveType::Bool => Some("__olive_bool_to_str"),
            OliveType::U64 => Some("__olive_str_u64"),
            _ => Some("__olive_str"),
        },
        "int" => match current_ty {
            OliveType::Float | OliveType::F32 => Some("__olive_float_to_int"),
            OliveType::Str => Some("__olive_str_to_int"),
            OliveType::PyObject => Some("__olive_py_to_int"),
            OliveType::Any => Some("__olive_unbox_int_checked"),
            OliveType::Union(_) if current_ty.is_tag_encoded_union() => {
                Some("__olive_unbox_int_checked")
            }
            _ => Some("__olive_int"),
        },
        "float" => match current_ty {
            OliveType::Float | OliveType::F32 => Some("__olive_copy_float"),
            OliveType::Int => Some("__olive_int_to_float"),
            OliveType::Str => Some("__olive_str_to_float"),
            OliveType::PyObject => Some("__olive_py_to_float"),
            OliveType::Any => Some("__olive_unbox_float_checked"),
            OliveType::Union(_) if current_ty.is_tag_encoded_union() => {
                Some("__olive_unbox_float_checked")
            }
            _ => Some("__olive_float"),
        },
        "bool" => {
            if matches!(current_ty, OliveType::Float | OliveType::F32) {
                Some("__olive_bool_from_float")
            } else if current_ty.is_tag_encoded_union() {
                Some("__olive_any_truthy")
            } else {
                Some("__olive_bool")
            }
        }
        "iter" => Some("__olive_iter"),
        "next" => Some("__olive_next"),
        "has_next" => Some("__olive_has_next"),
        "slice" => Some("__olive_str_slice"),
        // `list(pyobj)`/`dict(pyobj)` infer to `[Any]`/`{str: Any}` (unify.rs),
        // so elements must land boxed -- a raw native word here is bit-identical
        // to a small inline Any tag and gets misdecoded by any Any-aware reader.
        "list" => {
            if current_ty.is_py_value() {
                Some("__olive_py_to_any_list")
            } else {
                None
            }
        }
        "dict" => {
            if current_ty.is_py_value() {
                Some("__olive_py_to_any_dict")
            } else {
                None
            }
        }
        "keys" => Some("__olive_obj_keys"),
        "values" => Some("__olive_obj_values"),
        "remove" => Some("__olive_obj_remove"),
        "abs" => match current_ty {
            OliveType::Float | OliveType::F32 => Some("__olive_math_abs"),
            _ => Some("__olive_int_abs"),
        },
        "round" => Some("__olive_math_round_to_int"),
        "input" => Some("__olive_input"),
        _ => None,
    }
}

pub(crate) fn is_unsigned_op(func_mir: &MirFunction, op: &Operand) -> bool {
    match op {
        Operand::Copy(loc) | Operand::Move(loc) => matches!(
            func_mir.locals[loc.0].ty,
            OliveType::U8 | OliveType::U16 | OliveType::U32 | OliveType::U64 | OliveType::Usize
        ),
        _ => false,
    }
}

pub(crate) fn is_str_op(func_mir: &MirFunction, op: &Operand) -> bool {
    match op {
        Operand::Constant(Constant::Str(_)) => true,
        Operand::Copy(loc) | Operand::Move(loc) => func_mir.locals[loc.0].ty == OliveType::Str,
        _ => false,
    }
}

pub(crate) fn is_float_op(func_mir: &MirFunction, op: &Operand) -> bool {
    match op {
        Operand::Constant(Constant::Float(_)) => true,
        Operand::Copy(loc) | Operand::Move(loc) => {
            let ty = &func_mir.locals[loc.0].ty;
            matches!(ty, OliveType::Float | OliveType::F32)
        }
        _ => false,
    }
}

pub(crate) fn is_pyobj_op(func_mir: &MirFunction, op: &Operand) -> bool {
    match op {
        Operand::Copy(loc) | Operand::Move(loc) => func_mir.locals[loc.0].ty == OliveType::PyObject,
        _ => false,
    }
}

pub(crate) fn is_any_op(func_mir: &MirFunction, op: &Operand) -> bool {
    match op {
        Operand::Copy(loc) | Operand::Move(loc) => func_mir.locals[loc.0].ty == OliveType::Any,
        _ => false,
    }
}

pub(crate) fn is_list_op(func_mir: &MirFunction, op: &Operand) -> bool {
    match op {
        Operand::Copy(loc) | Operand::Move(loc) => {
            let ty = &func_mir.locals[loc.0].ty;
            matches!(
                ty,
                OliveType::List(_) | OliveType::Tuple(_) | OliveType::Set(_)
            )
        }
        _ => false,
    }
}

pub(crate) fn cl_type(ty: &OliveType) -> cranelift::prelude::Type {
    match ty {
        OliveType::Int | OliveType::U64 | OliveType::Usize | OliveType::Ptr(_) => types::I64,
        OliveType::I32 | OliveType::U32 => types::I32,
        OliveType::I16 | OliveType::U16 => types::I16,
        OliveType::I8 | OliveType::U8 | OliveType::Bool => types::I8,
        OliveType::Float => types::F64,
        OliveType::F32 => types::F32,
        OliveType::Vector(inner, width) => match &**inner {
            OliveType::Int | OliveType::U64 | OliveType::Usize => {
                types::I64.by(*width as u32).expect("invalid vector width")
            }
            OliveType::I32 | OliveType::U32 => {
                types::I32.by(*width as u32).expect("invalid vector width")
            }
            OliveType::I16 | OliveType::U16 => {
                types::I16.by(*width as u32).expect("invalid vector width")
            }
            OliveType::I8 | OliveType::U8 | OliveType::Bool => {
                types::I8.by(*width as u32).expect("invalid vector width")
            }
            OliveType::Float => types::F64.by(*width as u32).expect("invalid vector width"),
            OliveType::F32 => types::F32.by(*width as u32).expect("invalid vector width"),
            _ => types::I64,
        },
        _ => types::I64,
    }
}
