...
pub extern "C" fn olive_crypto_rsa_keygen() -> i64 {
    ...
    fields.insert(
        crate::OliveStringKey(olive_str_internal("pub")),
        olive_str_internal(&pub_b64),
    );
    fields.insert(
...
mod tests {
    ...
    fn rsa_keygen_returns_obj() {
        ...
        assert!(
            obj.fields
                .contains_key(&crate::OliveStringKey(olive_str_internal("pub")))
        );
        assert!(
            obj.fields
                .contains_key(&crate::OliveStringKey(olive_str_internal("priv")))
        );
        let pub_val = *obj
            .fields
            .get(&crate::OliveStringKey(olive_str_internal("pub")))
            .unwrap();
        let priv_val = *obj
            .fields
            .get(&crate::OliveStringKey(olive_str_internal("priv")))
            .unwrap();
        assert_ne!(pub_val, 0);
