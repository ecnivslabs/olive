...
use crate::python::{PyObject, olive_py_is_valid_proxy, olive_py_conv_to_py, olive_py_conv_to_olive};
...
pub static mut PY_TYPE_FROM_SPEC: unsafe extern "C" fn(*mut PyType_Spec) -> PyObject = noop_type_from_spec;
pub static mut PY_TYPE_GENERIC_ALLOC: unsafe extern "C" fn(PyObject, isize) -> PyObject = noop_alloc;

pub static mut PY_ERR_SET_STRING: unsafe extern "C" fn(PyObject, *const c_char) = noop_err_set_string;
...
unsafe extern "C" fn noop_type_from_spec(_: *mut PyType_Spec) -> PyObject { std::ptr::null_mut() }
unsafe extern "C" fn noop_err_set_string(_: PyObject, _: *const c_char) {}
unsafe extern "C" fn noop_alloc(_: PyObject, _: isize) -> PyObject { std::ptr::null_mut() }


pub unsafe fn setup_native_proxies(handle: *mut c_void, dlsym_fn: unsafe fn(*mut c_void, &str) -> *mut c_void) {
    ...
    if !exc_re.is_null() { PY_EXC_RUNTIMEERROR = *(exc_re as *mut PyObject); }
    
    let exc_ie = dlsym_fn(handle, "PyExc_IndexError");
    if !exc_ie.is_null() { PY_EXC_INDEXERROR = *(exc_ie as *mut PyObject); }

    let exc_ke = dlsym_fn(handle, "PyExc_KeyError");
    if !exc_ke.is_null() { PY_EXC_KEYERROR = *(exc_ke as *mut PyObject); }

    // Setup List Proxy
    let mut list_slots = vec![
        PyType_Slot { slot: 45, pfunc: list_proxy_len as *mut c_void }, // Py_sq_length
        PyType_Slot { slot: 44, pfunc: list_proxy_getitem as *mut c_void }, // Py_sq_item
        PyType_Slot { slot: 39, pfunc: list_proxy_setitem as *mut c_void }, // Py_sq_ass_item
        PyType_Slot { slot: 52, pfunc: proxy_dealloc as *mut c_void }, // Py_tp_dealloc
    ...
    let mut dict_slots = vec![
        PyType_Slot { slot: 4, pfunc: dict_proxy_len as *mut c_void }, // Py_mp_length
        PyType_Slot { slot: 5, pfunc: dict_proxy_getitem as *mut c_void }, // Py_mp_subscript
        PyType_Slot { slot: 3, pfunc: dict_proxy_setitem as *mut c_void }, // Py_mp_ass_subscript
        PyType_Slot { slot: 52, pfunc: proxy_dealloc as *mut c_void }, // Py_tp_dealloc
        PyType_Slot { slot: 0, pfunc: std::ptr::null_mut() },
    ];
...
unsafe extern "C" fn list_proxy_len(self_ptr: PyObject) -> isize {
    ...
    if !check_alive(proxy.ptr) { return -1; }
...
unsafe extern "C" fn list_proxy_getitem(self_ptr: PyObject, index: isize) -> PyObject {
    let proxy = &*(self_ptr as *const NativeProxy);
    if !check_alive(proxy.ptr) { return std::ptr::null_mut(); }
...
unsafe extern "C" fn list_proxy_setitem(self_ptr: PyObject, index: isize, value: PyObject) -> c_int {
    let proxy = &*(self_ptr as *const NativeProxy);
    if !check_alive(proxy.ptr) { return -1; }
    let length = crate::olive_list_len(proxy.ptr);
    let mut idx = index as i64;
...
unsafe extern "C" fn dict_proxy_getitem(self_ptr: PyObject, key: PyObject) -> PyObject {
    ...
    if key_str_obj.is_null() { return std::ptr::null_mut(); }
...
unsafe extern "C" fn dict_proxy_setitem(self_ptr: PyObject, key: PyObject, value: PyObject) -> c_int {
    let proxy = &*(self_ptr as *const NativeProxy);
    if !check_alive(proxy.ptr) { return -1; }
    
    let key_str_obj = crate::python::PY_OBJECT_STR(key);
    if key_str_obj.is_null() { return -1; }
    let key_utf8 = crate::python::PY_UNICODE_AS_UTF8(key_str_obj);
    if key_utf8.is_null() { 
        crate::python::PY_DEC_REF(key_str_obj);
        return -1; 
